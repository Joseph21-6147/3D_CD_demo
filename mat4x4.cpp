...
// Returns the (float) number stored at row r, column c in matrix m.
float Matrix_Peek( int r, int c, mat4x4 &m ) {
    return m.m[r][c];
}

// Sets the (float) number f at row r, column c in matrix m.
void Matrix_Poke( int r, int c, float f, mat4x4 &m ) {
    m.m[r][c] = f;
}

// Returns the row vector at row r from matrix m in a vec3d vector.
vec3d Matrix_GetRowVector( int r, mat4x4 &m ) {
    vec3d v;
    v.x = m.m[r][0];
    v.y = m.m[r][1];
    v.z = m.m[r][2];
...
// Returns the column vector at column c from matrix m in a vec3d vector.
vec3d Matrix_GetColumnVector( int c, mat4x4 &m ) {
    vec3d v;
    v.x = m.m[0][c];
    v.y = m.m[1][c];
    v.z = m.m[2][c];
...
// Transposes matrix m and returns it as a new matrix.
mat4x4 Matrix_Transpose( mat4x4 &m ) {
    mat4x4 matrix;
    for (int r = 0; r < 4; r++)
        for (int c = 0; c < 4; c++)
            matrix.m[r][c] = m.m[c][r];
    return matrix;
}
...
// alternative implementation - exactly the same function
mat4x4 Matrix_MultiplyMatrix2( mat4x4 &m1, mat4x4 &m2 ) {
    mat4x4 matrix;
    for (int c = 0; c < 4; c++)
        for (int r = 0; r < 4; r++)
            for (int i = 0; i < 4; i++)
                matrix.m[r][c] += m1.m[r][i] * m2.m[i][c];
    return matrix;
}
