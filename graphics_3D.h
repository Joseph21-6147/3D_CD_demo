...
#define RM_WIREFRAME_NOCULL  5    // Like RM_WIREFRAME, but without culling
#define RM_TEXTURED          6    // Textured      without wire frame drawing
#define RM_TEXTURED_PLUS     7    //               with     "     "      "
...
struct renderColour {
    // consoleGameEngine: store symbols that represent the colour
    wchar_t sym;
    short col;
...
public:
    ...
    // recalculates projection matrix with provided parameters
    void UpdateProjection( float fieldOfViewInDegrees, float nearPlaneDistance, float farPlaneDistance );
    ...
    void DisplayCameraSettings( int x, int y );
...
public:
    ...
    void CullViewAndProjectTriangle( triangle &inputTri, renderColour colInfo, std::vector<triangle> &vecOfTris );
...
private:
    ...
    void GetColour( float lum, triangle &tri );
    ...
    void GetColour3( float lum, renderColour colInfo, triangle &tri );
...
//struct cameraShake {
//    float traumaLevel = 0.0f;    // between 0.0f and 1.0f, damage or stress adds to trauma (0.2f or 0.5f)
//    float maxYaw      = CS_MAX_ANGLE;
//    float maxPitch    = CS_MAX_ANGLE;
//    float maxRoll     = CS_MAX_ANGLE;
//    float maxOffsetX  = CS_MAX_OFFSET;
//    float maxOffsetY  = CS_MAX_OFFSET;
//    float maxOffsetZ  = CS_MAX_OFFSET;
//};
//
//// global variable for now --> there must be a more elegant way to store this info ...
//extern cameraShake glbCamShakeInfo;
//
